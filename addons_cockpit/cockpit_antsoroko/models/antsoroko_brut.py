...
class AntsorokoBrut(models.Model):
    _name = 'antsoroko.brut'
    _description = 'Antsoroko Brut - Entrée/Sortie'

    date = fields.Date(string='Date')
    heure = fields.Datetime(string='Heure')
    vehicule = fields.Char(string='Véhicule')
    partenaire = fields.Char(string='Fournisseur/Client')
    ...
    @api.depends('date', 'type_mouvement', 'vehicule', 'quantite')
    def _compute_name(self):
        for rec in self:
            date_str = rec.date.strftime('%Y-%m-%d') if rec.date else ''
            rec.name = f"Brut {rec.type_mouvement or ''} {rec.vehicule or ''} {date_str} {rec.quantite or 0}Kg"
